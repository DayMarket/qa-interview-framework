...
class PayOrderTests extends BaseClientTest {
    ...
    @Test
    @DisplayName("Pay order returns 200 and status CREATED")
    void payOrderTest() {
        Response cancelOrderResponse = orderApi.payOrder(createdOrder.getOrderId());

        step("Проверить параметры созданного заказа", () -> {
            assertAll(
                    () -> assertThat(cancelOrderResponse.code())
                            .as("Expected HTTP status 201 when creating an order")
                            .isEqualTo(200),
                    () -> assertThat(createdOrder.getOrderId())
